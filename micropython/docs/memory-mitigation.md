...
- **Protocol Response Constants**: All fixed-length UART responses (`_RESP_OK`, `_RESP_CRC`, `_TIME_BUF`, `_RFM_RESP`, `_RESP_0xFF`, etc.) are pre-allocated at module level to eliminate heap allocation in the protocol hot path.
- **UART Receive Buffer**: A single 260-byte `bytearray` with `memoryview` is reused for all `readinto()` calls.
- **Bounded Deque Buffers**: `log_buffer` and `terminal_buffer` use `collections.deque(maxlen=N)` for automatic O(1) eviction, replacing the previous `list` + `pop(0)` pattern (which was O(n)).
