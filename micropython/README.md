...
| [💾 Memory Mitigation](docs/memory-mitigation.md) | RAM usage estimates and OOM prevention strategies. |
...
- **Pre-allocated Internal Buffers**: Critical protocol response constants (`_RESP_OK`, `_RESP_CRC`, `_TIME_BUF`, `_RFM_RESP`, etc.) and the UART receive buffer are pre-allocated at startup to eliminate heap allocation in the hot path.
- **Bounded Deque Buffers**: Log and terminal buffers use `collections.deque(maxlen=N)` for O(1) append/eviction instead of `list.pop(0)` which is O(n).
...
The DriveWire server includes host-side simulation tests to verify protocol correctness, RBF (OS-9) disk handling, web API behavior, and resilience logic without needing physical hardware.

### Running Tests

Use the master test runner for process-isolated execution of all test modules:

```bash
python3 run_all_tests.py
```

This runs all 4 test modules (`test_drivewire`, `test_resilience`, `test_web_api`, `test_os9_disk`) in separate subprocesses to prevent `sys.modules` pollution and `MagicMock` state leakage.
